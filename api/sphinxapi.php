...
class SphinxClient
{
	...
	function SetOverride ( $attrname, $attrtype, $values )
	{
		...
		assert ( in_array ( $attrtype, array ( SPH_ATTR_INTEGER, SPH_ATTR_TIMESTAMP, SPH_ATTR_BOOL, SPH_ATTR_FLOAT, SPH_ATTR_INTEGER ) ) );
		assert ( is_array ( $values ) );
		...
		foreach ( $this->_overrides as $key => $entry )
		{
			...
			foreach ( $entry["values"] as $id=>$val )
			{
				...
				switch ( $entry["type"] )
				{
					...
					case SPH_ATTR_INTEGER:	$req .= sphPackU64 ( $val ); break;
