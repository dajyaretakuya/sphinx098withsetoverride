//
// $Id: searchd.cpp 1217 2008-03-17 11:19:48Z shodan $
//
...
int SearchRequestBuilder_t::CalcQueryLen ( const char * sIndexes, const CSphQuery & q ) const
{
	...
	ARRAY_FOREACH(i, q.m_dOverrides)
		...
		(q.m_dOverrides[i].m_uAttrType == SPH_ATTR_INTEGER ? 16 : 12)*q.m_dOverrides[i].m_dValues.GetLength(); // ( bigint id; int/float/bigint value )[] values
	return iReqSize;
}
...
void SearchRequestBuilder_t::SendQuery ( const char * sIndexes, NetOutputBuffer_c & tOut, const CSphQuery & q ) const
{
	...
	ARRAY_FOREACH(i, q.m_dOverrides)
	{
		...
		ARRAY_FOREACH(j, tEntry.m_dValues)
		{
			...
			case SPH_ATTR_INTEGER:	tOut.SendUint64(tEntry.m_dValues[j].m_uValue); break;
...
bool ParseSearchQuery ( InputBuffer_c & tReq, CSphQuery & tQuery, int iVer )
{
	...
	if ( iVer>=0x113 ) {
		...
		ARRAY_FOREACH(i, tQuery.m_dOverrides)
		{
			...
			ARRAY_FOREACH(iVal, tOverride.m_dValues)
			{
				...
				else if (tOverride.m_uAttrType == SPH_ATTR_INTEGER)	tEntry.m_uValue = tReq.GetUint64();
	...
	if ( tReq.GetError() )
	{
		tReq.SendErrorReply ( "invalid or truncated request 2" );
		return false;
	}
...
void HandleCommandExcerpt ( int iSock, int iVer, InputBuffer_c & tReq )
{
	...
	for ( int i=0; i<iCount; i++ )
	{
		...
		if ( tReq.GetError() )
		{
			tReq.SendErrorReply ( "invalid or truncated request 1" );
			return;
		}
...
void HandleCommandUpdate ( int iSock, int iVer, InputBuffer_c & tReq, int iPipeFD )
{
	...
	if ( tReq.GetError() )
	{
		tReq.SendErrorReply ( "invalid or truncated request 3" );
		return;
	}
